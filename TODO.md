...
- [x] `❌ Не подходит` — добавление в blacklist + удаление сообщения + pop из buffer + send нового
- [x] Управление displayed-mapping (`message_id ↔ ad`)
- [x] Toast-уведомление через `answerCallbackQuery`
- [x] Тесты reject-flow

## Фаза 6c — Автостоп и пауза

- [x] Таймер автостопа (5 минут) — `asyncio.Task`
- [x] Reset таймера на каждом callback ("Не подходит")
- [x] `▶️ Возобновить` — кнопка после остановки
- [x] State `STOPPED_BY_TIMEOUT` vs `STOPPED_MANUALLY` (через `_stop_reason`)
- [x] Тесты автостопа

## Фаза 8 — Настройки / чёрный список

- [x] Раздел `⚙️ Настройки`
- [x] Просмотр черного списка описаний (одно сообщение на запись)
- [x] Удаление одной записи
- [x] `🗑 Очистить всё` с подтверждением
- [x] Тесты handlers
