...
### Ссылка «Купить» в отслеживании

По умолчанию бот отдаёт **прямой** HTTPS URL вида  
`https://www.bybit.com/{локаль}/p2p/{buy|sell}/{token}/{валюта}?itemId=…&adNo=…`  
(без `app.bybit.com/inapp`). Так Telegram чаще открывает **тот же маршрут**, что и браузер, и проще сработают Universal Links в приложение Bybit.

Если `www.bybit.com` у тебя недоступен, в `.env` выставь:

| Переменная | Значение |
|---|---|
| `BYBIT_P2P_LINK_MODE` | `inapp` — снова использовать обёртку `app.bybit.com` |
| `BYBIT_P2P_WEB_HOST` | например `www.bybitglobal.com`, если редиректишься на глобальный домен |
| `BYBIT_P2P_WEB_LOCALE` | сегмент локали в пути (`en-US`, `en`, `ru-RU`, …), если SPA открывает «не ту» версию сайта |

Официально Bybit не гарантирует экран карточки по этим параметрам — см. [docs/bybit-p2p-links.md](docs/bybit-p2p-links.md). Если снова видишь только список — открой ссылку **во внешнем браузере** (long-tap).
